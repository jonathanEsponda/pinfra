...
import {
  ...
  UsuarioModelEditarDatosPersonales,
} from "@/app/types";
...
export default function EditPerfilPage() {
  const {
    ...
    watch,
    reset,
  } = useForm<UsuarioModelEditarDatosPersonales>();
  ...
  useEffect(() => {
    async function fetchTelefono() {
      try {
        const res = await fetch("/api/telefonos"); // Endpoint que devuelve el teléfono del usuario actual
        if (res.ok) {
          const data: TelefonoModelResponse = await res.json();
          setTelefonoEstado(data);
        }
      } catch (error) {
  ...
  const onSubmit = handleSubmit(
    async (data: UsuarioModelByIdAndModifyWithPhone) => {
      ...
      const usuarioData: UsuarioModelByIdAndModify = {
        ...
        activo: data.activo,
        estado: data.estado,
        // Se incluye el campo contrasenia solo si se ingresó un valor no vacío
